...
def build_jianying_template_files(
    ...
    width = as_int(video_metadata["width"])
    height = as_int(video_metadata["height"])
    duration_us = as_int(as_float(video_metadata["duration"]) * MICROSECONDS)
    ...
    for index, item in enumerate(timeline):
        ...
        start_us = as_int(as_float(item["start"]) * MICROSECONDS)
        cue_duration = as_int((as_float(item["end"]) - as_float(item["start"])) * MICROSECONDS)
        segment.update(
            {
