...
此模块属于 Core，不能导入 WebUI 或 ``app.services``。文本契约与产品确认的
Streamlit“剧情理解”能力保持一致，但渲染、调用与校验均由 Core 独立完成。
"""

PLOT_ANALYSIS_PROMPT_CATEGORY = "short_drama_narration"
PLOT_ANALYSIS_PROMPT_NAME = "plot_analysis"
PLOT_ANALYSIS_PROMPT_VERSION = "v1.1"

PLOT_ANALYSIS_SYSTEM_PROMPT = (
    "你是一位专业的短剧解说策划和剧本分析师。"
    "请输出克制、结构化、可直接供下游解说脚本生成使用的剧情理解材料。"
)

PLOT_ANALYSIS_TEMPLATE = """# 角色
...
def render_plot_analysis_prompt(subtitle_content: str) -> str:
    """使用 Core 自有的确定性替换器渲染剧情理解 Prompt。"""

    return PLOT_ANALYSIS_TEMPLATE.replace("${subtitle_content}", subtitle_content)


def plot_analysis_prompt_metadata() -> dict[str, str]:
    """返回可持久化且不含秘密的 Prompt 版本信息。"""

    return {
        "prompt_category": PLOT_ANALYSIS_PROMPT_CATEGORY,
        "prompt_name": PLOT_ANALYSIS_PROMPT_NAME,
        "prompt_version": PLOT_ANALYSIS_PROMPT_VERSION,
    }
