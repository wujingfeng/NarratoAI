...
def test_upgrade_0003_backfills_existing_dispatch_and_response(tmp_path):
    ...
    with engine.begin() as connection:
        for task_id, status, version in (
            ...
            connection.execute(
                ...
                {**base, "id": task_id, "scope": task_id, "status": status, "version": version},
            )
        connection.execute(
            text(
        ...
        connection.execute(
            text("UPDATE core_tasks SET current_attempt_no=1 WHERE id='ctask_legacy_running'")
        )
    ...
    with Session(engine, expire_on_commit=False) as session:
        ...
        for task_id in ("ctask_legacy_queued", "ctask_legacy_retry", "ctask_legacy_running"):
            task = session.get(CoreTask, task_id)
            assert task.initial_response == {"core_task_id": task_id, "status": "queued"}
        recorder = Recorder()
        assert DispatchOutboxPublisher(session).publish_pending(
