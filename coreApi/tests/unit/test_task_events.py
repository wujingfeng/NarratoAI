...
def test_task_events_are_authenticated_ordered_and_in_openapi(
    app, client, settings
):
    ...
    task = task_service.create_core_task(
        ...
        idempotency_key="events-probe", input_snapshot={"asset": "one"}
    )
    attempt = task_service.start_attempt(task.id)
    task_service.complete_attempt(attempt.id, attempt.lease_token, [], lease_version=attempt.lease_version)

    app.dependency_overrides[get_database_session] = lambda: database_session
    path = f"/api/v1/tasks/{task.id}/events"
    assert path.replace(task.id, "{core_task_id}") in client.get("/openapi.json").json()["paths"]
    ...
    assert all(set(event) == {
        "event_id", "sequence", "event_type", "time", "status", "phase",
        "progress", "attempt", "error", "artifacts"
    } for event in events)
    assert client.post(path, headers={"Authorization": "Bearer test-service-token"}).status_code == 405
...
def test_task_query_openapi_publishes_versionable_dto_schema(client):
    ...
    task_schema = document["paths"]["/api/v1/tasks/{core_task_id}"]["get"]["responses"]["200"]["content"]["application/json"]["schema"]
    event_schema = document["paths"]["/api/v1/tasks/{core_task_id}/events"]["get"]["responses"]["200"]["content"]["application/json"]["schema"]
    assert "$ref" in task_schema
    assert "$ref" in event_schema
    schemas = document["components"]["schemas"]
    for name in ("CoreTaskArtifactDTO", "CoreTaskDTO", "CoreTaskEventDTO", "CoreTaskEventsDTO"):
