...
from core_api.tasks.models import CoreTaskAttempt, CoreTaskStatus, utc_now
...
def test_stale_attempt_cannot_complete_task(task_service, task, session):
    ...
    assert task.current_attempt_no == second.attempt_no
...
def test_retry_limit_means_initial_plus_three_automatic_retries(task_service):
    ...
    for expected_no in (2, 3, 4):
        ...
        attempt = task_service.expire_and_restart(attempt.id)
        assert attempt.attempt_no == expected_no

    attempt.lease_expires_at = utc_now() - timedelta(seconds=1)
    task_service.session.commit()
    assert task_service.expire_and_restart(attempt.id) is None
...
def test_retryable_failure_preserves_error_and_restarts(task_service, task, session):
    """临时错误保留规范化错误证据并进入新 attempt。"""
