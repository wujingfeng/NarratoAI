...
def _subtitle_dir() -> str:
    """返回不依赖 Streamlit 应用模块的可写字幕目录。"""
    root = os.environ.get("NARRATO_RESOURCE_DIR", "").strip()
    if not root:
        root = os.path.join(os.getcwd(), "resource")
    directory = os.path.join(root, "srt")
...
def write_srt_file(srt_content: str, subtitle_file: str = "") -> str:
    if not subtitle_file:
        subtitle_file = os.path.join(_subtitle_dir(), f"fun_asr_{int(time.time())}.srt")
