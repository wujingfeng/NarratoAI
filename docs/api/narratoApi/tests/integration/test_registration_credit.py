...
def test_successful_registration_commits_exactly_one_signup_bonus() -> None:
    ...
    codes = EmailCodeManager(InMemoryEmailCodeStore(), secret="test-secret", ttl_seconds=600)
    service = AuthService(
    ...
    user = service.register("signup@example.com", "Correct-Horse-Battery-42", issue.code)
    ...
    with sessions() as session:
        ...
        assert [(entry.entry_type, entry.amount, entry.idempotency_key) for entry in entries] == [
