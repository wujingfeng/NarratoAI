...
def _workflow_service() -> tuple[WorkflowService, sessionmaker]:
    ...
    with sessions.begin() as session:
        session.add(
            User(id="usr_workflow", email="workflow@example.com", password_hash="hash")
        )
        session.add(
            Project(
                id="prj_workflow",
...
def test_transition_persists_state_and_a_single_idempotent_outbox_event() -> None:
    ...
    assert [
        (event.event_type, event.idempotency_key, event.status, event.payload)
        for event in outbox
    ] == [
        (
