...
def test_waiting_for_edit_has_no_automatic_timeout_or_failure_transition() -> None:
    assert transition_workflow_state("waiting_for_edit", "failed", actor="system") is False
    assert transition_workflow_state("waiting_for_edit", "waiting_for_edit", actor="system") is True


def test_terminal_workflow_states_cannot_be_recovered() -> None:
    for terminal_state in ("completed", "failed"):
        assert transition_workflow_state(terminal_state, "running", actor="system") is False


def test_only_retryable_failed_attempt_with_remaining_budget_is_a_retry_candidate() -> None:
