...
def project_for_update_statement(*, user_id: str, project_id: str):
...
class UploadService:
    ...
    def complete(
        ...
        *,
        ...
        try:
            ...
            raise ApiError("OSS_UNAVAILABLE", "Upload object verification is unavailable", 503) from error
    ...
    def reserve(self, *, user_id: str, project_id: str, asset_type: str, filename: str, size_bytes: int, object_key: str, cdn_url: str) -> None:
        ...
        with self.session_factory() as session:
            ...
            try:
                ...
                raise ApiError("UPLOAD_DECLARATION_REJECTED", "Upload declaration is invalid", 422) from error
            session.add(Asset(id=new_asset_id(), user_id=user_id, project_id=project.id, asset_type=asset_type, status="validating", filename=filename, bucket=self.oss_bucket, object_key=object_key, cdn_url=cdn_url, size_bytes=size_bytes, reservation_expires_at=now + POLICY_RESERVATION_TTL))
            session.commit()
    ...
    def get_owned_asset(self, *, user_id: str, asset_id: str) -> Asset:
        ...
        with self.session_factory() as session:
            ...
            if persisted is None:
                raise ApiError("ASSET_NOT_FOUND", "Asset not found", 404)
            if persisted.status == "validating" and persisted.core_task_id == core_task_id:
                persisted.status = "ready" if probe.valid else "invalid"
                session.commit()
                session.refresh(persisted)
            return persisted
