...
from narrato_api.assets.schemas import AssetData, UploadCompleteRequest, UploadPolicyData, UploadPolicyRequest
...
def get_oss_client(request: Request, settings: Annotated[Settings, Depends(get_settings)]) -> HttpOssClient:
    """创建仅供当前请求 HEAD 和 URL 构造的 OSS 客户端。"""

    return HttpOssClient(endpoint=settings.oss_endpoint)


def get_core_client(settings: Annotated[Settings, Depends(get_settings)]) -> HttpCoreClient:
    """创建只暴露媒体探测的 Core 调用客户端。"""

    return HttpCoreClient(base_url=str(settings.core_base_url), request_token=settings.core_request_token)
...
def get_upload_service(
    ...
    return UploadService(
        session_factory=sessionmaker(bind=request.app.state.database_engine, expire_on_commit=False),
        oss_client=oss_client, core_client=core_client, oss_bucket=settings.oss_bucket,
    )


@router.post("/projects/{project_id}/uploads/policy", response_model=ApiResponse[UploadPolicyData])
def create_upload_policy(
    project_id: str, body: UploadPolicyRequest,
...
@router.post("/projects/{project_id}/uploads/complete", status_code=status.HTTP_202_ACCEPTED, response_model=ApiResponse[AssetData])
def complete_upload(
    project_id: str, body: UploadCompleteRequest,
    token: Annotated[str, Depends(bearer_token)],
    auth: Annotated[AuthService, Depends(get_auth_service)],
    service: Annotated[UploadService, Depends(get_upload_service)],
    request_id: Annotated[str, Depends(get_request_id)],
