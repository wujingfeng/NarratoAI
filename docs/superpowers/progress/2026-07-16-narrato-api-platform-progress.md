...
- **Commit：** R6 最终提交 `f016815`，提交主题 `feat: scaffold narrato business api`；R6 仅更新本进度日志并 amend 同一 Task 10 提交。
...
- **邮件与 API：** 注册码和找回码分别由 Celery 任务异步投递，API 仅在 Broker 接受后返回 `202`；Broker 载荷使用随机 nonce、HMAC 派生流和认证标签密封验证码，任务消息不出现六位明文。MailClient Protocol、Fake dispatcher 和 SMTP STARTTLS/timeout/header-injection 边界已实现，代码与凭据不写日志。新增计划要求的 7 个 GET/POST 接口，全部使用 Task 10 typed envelope 和 extra-forbid DTO；认证失败统一 `401 AUTHENTICATION_REQUIRED`。
- **GREEN：** Task 11 Fake 定向 `15 passed, 2 skipped, 1 warning`；隔离真实 Redis 7.2 定向 `17 passed, 1 warning`，确认生产 Lua 并发消费只有一个成功、单会话替换、摘要键值、固定 TTL 和 resolve 不续期；narratoApi 全量 `47 passed, 2 skipped, 1 warning`。Ruff、Mypy strict 22 files、pip check、compileall、`git diff --check` PASS。既有数据库与 fresh SQLite `alembic upgrade head/check` PASS；PostgreSQL offline DDL 含 users 表、规范化邮箱/状态 CHECK 和唯一邮箱索引。direct wheel 与 sdist→wheel 分别安装到两个仓外干净 Python 3.12 venv，Argon2id hash/verify、auth OpenAPI import/create_app 和 pip check 均 PASS。Core 全量 `384 passed, 12 warnings`；legacy `130 passed, 1 skipped, 4 warnings`。
- **计划偏差：** design 要求的新用户默认 100 创作点属于 Task 12 账本迁移与注册事务扩展，本 Task 只建立账户并明确保持 staged boundary，未提前创建积分表或伪造余额。真实 SMTP 无凭据与运行条件，邮件使用 Fake 完成必需自动化；Redis 除 Fake 外已用本机隔离实例验证生产 Lua，其他真实依赖 Smoke 留待 Gate B/D 条件具备时执行。
- **剩余风险：** Celery Broker 接受不等于 SMTP 最终送达；可靠邮件 Outbox 与全局限流属于后续可靠性/运维范围。生产 Redis Lua 已通过本机隔离 Redis 7.2 自动化，真实 SMTP 网络链路仍因无凭据未验证；接口、失败收敛和 Fake/真实 Redis 并发语义均有自动化证据。

### Task 11 独立审查 R1 修复

- **R1 RED：** 独立审查发现 2 Critical + 4 Important。新增 password-version fencing、两种 login/reset barrier 顺序、Redis revoke 失败回滚、DB version 已提交但旧 Redis Token 残留、干净 Celery Worker 任务注册、发送接口账户枚举、并发验证码 generation/Broker 失败、Redis Cluster keyslot/Lua 静态边界和认证/SMTP readiness 测试；首次专项为 `22 failed, 40 passed, 2 skipped`，逐项复现全部审查问题。
- **密码版本栅栏：** Redis 会话事实现在保存 `user_id + password_version`，原始 Token 仍只以 SHA-256 摘要作为键；每次 resolve 都读取业务数据库核对 `active` 与单调密码版本，不匹配时删除该摘要会话并返回统一 401。login 和 reset 均使用进程内固定条带锁及数据库 `SELECT FOR UPDATE`：login 在行锁事务中完成密码验证和带版本 Redis issue 后才释放；reset 先可靠 revoke，失败即回滚且不更新密码，再写入 Argon2id hash、递增版本并提交。disable 同样先 revoke、递增版本后提交。两种 barrier 顺序、revoke 503 不改密码，以及模拟 commit 后旧 Token 残留均已自动化闭合。
- **验证码与不可枚举发送：** 验证码 generation 改为 Redis `SET NX EX`；未过期 generation 不旋转、不重复发送，因此并发请求只有首个创建者 enqueue，所有实际发送邮件中的 code 都仍是 current。Broker 拒绝通过 HMAC compare-delete 只撤销本 generation；Celery task 重试沿用同一 sealed code。注册发送对已存在邮箱、找回发送对不存在/禁用邮箱均返回相同 202 envelope，并投递不含邮箱的 cover task，使 Redis/Broker 工作量近似且不产生可识别邮件。
- **Cluster-safe Redis：** 所有认证键统一固定 `{narrato-auth}` hash tag；所有 Lua 只访问调用方显式声明的 KEYS，禁止 ARGV 拼接动态键。session replace 先乐观读取 user current hash，再以 user/new-session/old-session 三个同槽 KEYS 做有限 CAS 重试；resolve/logout/revoke-user 同样使用显式同槽 KEYS 与匹配删除。keyslot 与脚本静态探针、隔离真实 Redis 并发 CAS/TTL/摘要测试均通过。
- **邮件 Worker 与 readiness：** Celery app 显式 include/import/finalize `narrato_api.auth.tasks`，全新解释器及构建产物只导入 worker app 即能看到邮件与 cover 两项自有任务。readiness 现在强制非占位且至少 32 字符的验证码 HMAC secret、SMTP host/from、STARTTLS、成对认证凭据和任务注册事实；缺失配置公开稳定 503。HMAC 与 SMTP password 均保持 `repr=False` 并进入 Task 10 动态日志脱敏 registry。
- **R1 边界：** 注册赠送 100 点仍严格留在 Task 12；未新增 refresh token、用户取消或其他非范围能力。真实 SMTP 因无凭据仍为可选 Smoke；可靠邮件 Outbox 属于后续事务可靠性范围，本 Task 已保证 Broker 接受前不返回首个 generation 成功且失败安全撤销。
- **R1 GREEN：** narratoApi Python 3.12 全量 `64 passed, 2 skipped, 1 warning`；Task 11 邮箱码、单点会话与注册登录在隔离真实 Redis 7.2 下 `25 passed, 1 warning`。Ruff、Mypy strict 22 files、pip check、compileall、`git diff --check` 均 PASS；fresh SQLite `upgrade head + alembic check`、PostgreSQL offline DDL 均 PASS。direct wheel 与 sdist→wheel 分别安装到仓库外干净 venv，从 `/tmp` 创建应用、核对 auth OpenAPI 与两项 Celery Worker 任务注册，两个环境 `pip check` 均 PASS。Core 全量 `384 passed, 12 warnings`；legacy `130 passed, 1 skipped, 4 warnings`。
- **R1 审查状态：** 2 Critical + 4 Important 已逐项补测试、修复并完成上述验证；当前等待独立 R2 需求与代码复审，不在复审前宣称 Task 11 正式收口。

### Task 11 独立审查 R2 修复

- **R2 RED：** 独立复审发现 1 Critical + 5 Important：Web 首次请求会经 `shared_task`/default app 发往默认 AMQP；验证码用 `SET NX` 禁止十分钟内重发且迟到任务无 generation fence；请求级 AuthService 的条带锁不共享；readiness 未探测 Celery Broker；邮箱接受首尾/连续点号；cover task 的任务名、载荷和 Redis 往返仍形成账户 oracle。复审的 direct/sdist Web-first、590 秒重发、乱序任务、关闭 Broker、无效邮箱和双分支形状探针均作为本轮 RED 基线。
- **App-owned Producer：** 移除 `shared_task` 与 current/default app 依赖。每个 FastAPI lifespan 按当前 `Settings` 创建并持有独立 Celery app，`CeleryMailDispatcher` 只对该实例调用固定任务名 `send_task`；退出时关闭 producer 连接池。Worker 由 `register_auth_tasks(app)` 显式注册同一任务。不同 broker Settings 的双 app、全新解释器以及 direct wheel/sdist→wheel Web-first 均确认 write URI 精确指向各自 Redis Broker，且不存在 cover task。
- **Generation 状态机：** Redis 验证码值仅保存 HMAC digest、随机 generation 与 `pending/sending/sent`，所有 Lua 只访问显式同槽 KEYS。pending/sent 重发原子替换 generation 并把 TTL 重置到配置值；sending 暂不旋转。Worker 在 SMTP 前 claim 精确 generation，stale/过期任务直接跳过，成功标 sent，SMTP/Redis 暂态失败释放或自动重试；Celery 消息设置 TTL `expires` 和绝对截止时间。真实 Redis 验证重发后旧码失效、TTL 约 600 秒、A/B 乱序只发送 current B，以及 sending/release/再次 rotate。SMTP 有效分钟由配置 TTL 向上取整，不再硬编码 10。
- **不可枚举同形路径：** 删除 cover task。存在/不存在账户均执行同一 DB 查询、同一 Redis rotate、同一 HMAC 密封和同一 `narrato.auth.send_verification_email` task，kwargs 键及长度形状一致；`deliver` 位包含在认证密封载荷内，Worker claim 状态机后才决定是否调用 SMTP。HTTP status/body 继续完全一致，任务名和明文 Broker kwargs 不暴露账户分支、邮箱验证码或 deliver 事实。
- **事务与应用所有权：** 条带锁提升为 lifespan 级 `AccountLockRegistry`，所有生产请求共享；其定位仅是 SQLite/单进程测试兼容，生产正确性继续由 `SELECT ... FOR UPDATE` 行锁事务保障。PostgreSQL dialect 探针确认账户查询编译含 `FOR UPDATE`，login/reset barrier 与故障测试继续证明 Redis issue/revoke 在事务 commit 前执行且版本 fencing fail closed，未将 SQLite 结果冒充跨进程 PostgreSQL 实测。
- **Broker readiness 与邮箱：** readiness 在同一总 deadline/有界 executor 中新增独立 Celery write connection/channel 探针，即使认证 Redis 可用而 Broker 关闭也返回稳定 503；探针不发布任务且 finally 关闭 channel/connection/app。邮箱改用声明依赖 `email-validator`，禁 SMTPUTF8/quoted local、禁非 ASCII/IDNA 歧义及 254 字符以上输入，统一 trim/lower；注册、登录、找回 HTTP 均拒绝首点、尾点和连续点。
- **R2 GREEN：** narratoApi Python 3.12 全量 `74 passed, 5 skipped, 1 warning`；启用隔离真实 Redis 7.2 的 Task 11 专项 `36 passed, 1 warning`。Ruff、Mypy strict 22 files、pip check、compileall、`git diff --check` PASS；fresh SQLite upgrade/check、PostgreSQL offline DDL PASS。direct wheel 与 sdist→wheel 均在仓外干净 venv 验证 `email-validator` 依赖、Web-first custom broker URI、Worker 注册、无 cover task、OpenAPI/邮箱边界及 pip check。Core 最终 `384 passed, 12 warnings`；legacy `130 passed, 1 skipped, 4 warnings`。Core 首轮在受限沙箱因 ProcessRunner 无权向自身子进程组发信号出现 1 fail，单测重跑通过，按该测试真实权限要求在沙箱外全量重跑后 384 全绿，未修改 Core 代码。
- **R2 审查状态：** 本轮 1 Critical + 5 Important 均已有实现和自动化证据，等待独立 R3 复审；真实 SMTP 仍按 Goal 作为无凭据可选 Smoke，不声明已验证外部邮件供应商。

### Task 11 独立审查 R3 修复

- **R3 RED：** 独立复审确认 R2 的 producer、重发、app-scoped lock、Broker readiness、邮箱和同形任务主路闭合，但发现 3 个 Important：claim 后硬 Worker loss 会令 generation 整个 TTL 卡在 `sending`；custom Celery app 的任务仍读取全局 cached Settings；Broker readiness 在 `connection.close()` 抛错时跳过 `app.close()`。真实 Redis worker-loss、custom HMAC/Redis worker 和多阶段 cleanup fault injection 作为本轮 RED 基线。
- **可恢复 claim lease：** `sending` 现携带随机 `claim_id` 与 `lease_until`。Lua claim 支持 pending 首次领取、active lease 返回 busy+剩余秒数、过期 lease 原子 takeover；rotate 仅在 active sending 时保持当前 accepted，lease 过期可替换为新 generation。任务改为 `bind=True`，active duplicate 不再正常 return/ACK，而是 `self.retry(countdown=lease remaining)`；Celery 继续 `acks_late + reject_on_worker_lost`。mark sent 与 release 均须精确匹配 generation、digest、claim_id，旧 owner 无法覆盖新 owner。真实 Redis 模拟 claim 后进程硬丢失，lease 过期 redelivery 成功发送 current；active duplicate 只产生延迟 retry、不双发，错误 owner release 失败。
- **超时安全关系：** 新增 `verification_code_send_lease_seconds`，Settings 模型与 readiness 均强制 SMTP timeout、认证 Redis read timeout 小于 lease，lease 小于验证码 TTL；默认 `10s SMTP < 30s lease < 600s TTL`。Redis transition 失败继续进入 Celery 自动重试；无主 sending 会在 lease 后由 redelivery 接管，不再永久卡死。
- **Worker 配置所有权：** `create_celery_app(settings)` 现在调用 `register_auth_tasks(app, settings)`，任务闭包只使用该 app 创建时持有的不可变 Settings，完全移除 `get_cached_settings()`。custom app 的 Redis URL/key prefix、HMAC、SMTP、TTL 和 lease 均与其 producer/worker 配置一致；同进程 global/custom app 注册表互不串配置，custom sealed payload 已在真实 Redis Worker 任务中成功解密和发送。
- **Readiness failure-safe cleanup：** Broker probe 对 `connection_for_write`、ensure、channel、channel close、connection close、app close 建立独立 `BaseException` cleanup 边界；前一步失败仍继续后续清理，原业务/连接异常优先传播，只有无主异常时才传播第一个 cleanup error。故障注入确认 connection close 与 app close 同时失败仍调用 app close；ensure 抛 `KeyboardInterrupt` 且两个 cleanup 均失败时仍传播同一原始对象且资源清理全部被尝试。
- **R3 GREEN：** narratoApi Python 3.12 全量 `78 passed, 6 skipped, 1 warning`；启用隔离真实 Redis 7.2 的 Task 11 专项 `37 passed, 1 warning`。Ruff、Mypy strict 22 files、pip check、compileall、`git diff --check` PASS；fresh SQLite upgrade/check 与 PostgreSQL offline DDL PASS。direct wheel 与 sdist→wheel 分别安装到仓外干净 venv，两个包均用 custom Redis/HMAC/SMTP Settings 在隔离 Redis 上成功解密 sealed payload、claim generation 并调用 Fake SMTP，且 `pip check` PASS。Core `384 passed, 12 warnings`；legacy `130 passed, 1 skipped, 4 warnings`。真实 SMTP 继续属于无凭据可选 Smoke。
- **R3 审查状态：** 3 个 Important 已完成实现与故障/恢复测试，等待独立 R4 复审；不在复审前宣称 Task 11 正式完成。

### Task 11 独立审查 R4 修复

- **R4 RED：** 独立复审确认 R3 的 claim lease、custom Worker Settings 和 readiness cleanup 闭合，但发现 2 个 Important：rotate/claim 仍信任 Web/Worker 本地 epoch，节点时钟偏差可立即 takeover；SMTP 多次 socket 操作没有总 deadline 且发送期间不续租，慢发送超过固定 lease 后可让新码先发、旧码后发并导致最后邮件失效。真实 Redis clock-skew 与 blocking SMTP 乱序探针作为本轮 RED 基线。
- **Redis TIME 唯一事实源：** rotate、claim、busy lease remaining、takeover 和 renew Lua 均在 Redis 内调用 `TIME` 计算毫秒时钟；客户端不再向 FSM 传入 epoch，任务也移除 Web 生成的绝对 `expires_at` 判定，generation 有效性只由 Redis key TTL/current 状态决定。真实 Redis 在 Python/Web 时间被改到极端快钟后仍拒绝 rotate，第二 owner 仍只得到 busy，当前 owner renew 成功；源码探针确认 Redis store 无 `time.time` 且所有包含 `now_ms` 的 Lua 都调用 Redis `TIME`。
- **受治理 SMTP 子进程：** 新增 `python -m narrato_api.auth.smtp_sender`，父 Worker 用固定无 secret argv、`shell=False`、`start_new_session=True` 启动；SMTP 配置、收件人和 code 只经有界 stdin JSON 传递，stdout/stderr 直接进入 `DEVNULL` 且从不记录。父进程施加单次总 deadline，超时或 owner/renew 丢失时向整个进程组 TERM、宽限后 KILL 并触发 Celery retry；不使用 daemon thread。`MailClient`/`SmtpMailClient` 仍由子进程复用，Fake 边界保持可测试。
