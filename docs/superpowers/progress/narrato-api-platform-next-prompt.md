...
- 最近 Implementation Commit：`a56de56 feat: guard failed project artifact visibility`。
- Task 14A--14E、Gate review、Task 15A、15B、15C、15D 已完成。
- Task 15D 只有产物可见性服务守卫：`completed` 保留已登记产物；`failed` 和任何其他状态返回空列表；不创建产物实体或导出能力。
- 验证：RED 为 `visible_artifacts` 缺失；最终直接复验 `15 passed in 0.02s`；Ruff、`git diff --check` 通过。
...
只实现 **completed-project export eligibility guard**。严格 TDD：先写测试并观察预期 RED，再以最小代码使其 GREEN。验收必须证明只有 `completed` 项目可导出，`failed` 和所有非完成状态不可导出，同时不创建产物实体、不生成导出/Jianying、不实现 router、Core 调用、下游行为或第二个 Task 15 子任务。完成 checkpoint 后立即停止。
...
3. 只读取 Task 15E 所需计划/设计章节和相关测试；不得无目的读取完整大文档或成功日志。
4. 完成当前子任务后运行目标测试、检查 diff、提交业务代码和测试；随后更新并提交三个永久进度文件为独立 checkpoint。
5. checkpoint 后检查 `git status --short`；任何预期未提交改动必须逐项记录在状态文件中。

## 最终回复要求

报告完成的 Task 15E 子任务、Implementation Commit、Checkpoint Commit、测试结果、当前 Gate、下一个原子任务和工作区状态。若仍有未完成任务，必须明确输出：
