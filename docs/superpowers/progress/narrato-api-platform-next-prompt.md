...
- 最近 Implementation Commit：`9a0d304 feat: guard non-terminal project deletion`。
- Task 14A--14E、Gate review、Task 15A、15B、15C 已完成。
- Task 15C 只有删除资格守卫：`completed`/`failed` 允许，其他项目状态为 `PROJECT_NOT_TERMINAL`；不执行删除。
- 验证：RED 是缺少 projects service；最终直接项目复验 `24 passed, 2 warnings`（早期直接集 `23 passed`）；Ruff、`git diff --check` 通过。
...
只选择一个独立的 Task 15 边界，优先实现 **失败项目产物可见性守卫**。严格 TDD：先写测试并观察预期 RED，再以最小代码使其 GREEN。验收必须证明 failed 项目没有任何可见产物，同时不创建产物实体、不实现导出/Jianying、router、Core 调用、下游行为或第二个 Task 15 子任务。完成 checkpoint 后立即停止。
...
3. 只读取 Task 15D 所需计划/设计章节和相关测试；不得无目的读取完整大文档或成功日志。
4. 完成当前子任务后运行目标测试、检查 diff、提交业务代码和测试；随后更新并提交三个永久进度文件为独立 checkpoint。
5. checkpoint 后检查 `git status --short`；任何预期未提交改动必须逐项记录在状态文件中。

## 最终回复要求

报告完成的 Task 15D 子任务、Implementation Commit、Checkpoint Commit、测试结果、当前 Gate、下一个原子任务和工作区状态。若仍有未完成任务，必须明确输出：
