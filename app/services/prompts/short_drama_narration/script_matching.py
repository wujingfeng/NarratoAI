...
3. 每个 OST=0 item 只能承载一个可独立朗读的短句；可以压缩相邻候选句，但每个 item 最多 60 个非空白字符，不能把整段文案塞进一个 item。
4. 为每个解说片段寻找最匹配的原始字幕画面，优先选择能表达该句核心含义的画面。
5. 使用公式估算所需画面时长：所需秒数 = 解说字数 / 5。匹配画面时长尽量接近，误差优先控制在 ±0.5 秒。
6. 每个 OST=0 item 的 timestamp 最长 12 秒。如果一句解说超过 60 个非空白字符或需要超过 12 秒，必须按语义拆成多个 item，分别匹配不同或连续画面。
7. timestamp 必须取自对应 video_id 的原始 SRT cue 时间范围；允许合并相邻 cue，但不得虚构超出字幕窗口的时间，也不得换算为多个视频拼接后的累计时间。
8. 同一 video_id 内时间段不得交叉或重叠。
9. 第一段必须是 OST=0 解说钩子，不能直接播放原片。
10. OST=1 原声片段的数量占比要尽量接近用户选择的 ${original_sound_ratio}%。计算方式：OST=1 的 items 数量 / 全部 items 数量。不要按 timestamp 总时长计算。
11. 不要自行判断或改写短剧类型；画面匹配和 picture 描述要服务用户选择的 ${drama_genre} 叙事重点。
12. 每段 OST=1 的 timestamp 不超过 10 秒。
13. 禁止用单个 item 覆盖整集、整个源视频或一整段长文案；最终 items 必须是可直接用于剪辑和逐条生成字幕的细粒度脚本。
...
- 如果原片占比与“第一段必须 OST=0”冲突，优先保证第一段是 OST=0，然后在后续片段提高 OST=1 数量占比。
- 选择高原片占比时，可以把用户文案合并成更少的 OST=0 桥段，不要为了逐句使用文案而压低原片占比。
- 因为第一段必须 OST=0、剧情连贯性或字幕素材限制，OST=1 数量占比允许有 ±1 个片段误差。
- 不要为了凑原片占比把 OST=1 切成无意义短片段；每个 OST=1 应尽量承载完整对白、关键动作或情绪节点，通常不少于 3 秒。

## OST 连续性规则
- ** 当 ${original_sound_ratio}% > 40% 时，禁止连续 5 个或更多 OST=0 解说片段。**
- OST=1 片段应优先选择关键对白、反转、情绪爆发、身份揭露、打脸爽点或剧情转折，不要插入无意义空镜。
- 当 ${original_sound_ratio}% 在 70%-90% 时，连续 OST=0 建议不超过 2 个。
- 如果 ${original_sound_ratio}% = 0%，本规则不适用。
