...
11. OST=0 文案必须匹配画面时长，按“解说字数 / 5 = 所需视频秒数”估算；每个 item 最多 60 个非空白字符且 timestamp 最长 12 秒，超过任一限制必须按语义拆成多个 item。
12. 不要自行改判短剧类型；如需改写 narration，必须按用户选择的 ${drama_genre} 保持表达重点。
13. 尽量保留原脚本中没有错误的片段；无法修复的片段可以删除，但剩余片段必须重新按 1 开始编号。
14. 遇到 SCRIPT_NARRATION_ITEM_TOO_LONG 或 SCRIPT_NARRATION_SEGMENT_TOO_LONG 时，必须使用可用 SRT cue 的真实时间窗口拆分，禁止把整集、整个源视频或整段文案保留为一个 item。
